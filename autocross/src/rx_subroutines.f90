...
subroutine rx_subroutine(nx, ny, nout, tx, x, ty, y, &
  ...
  ini = .true.
  !
  print *, 'ny, ty(1), ty(npy)=', ny, ty(1), ty(ny)
  print *, 'call calc_nyquist_param from rx_subroutine'
  
  call calc_nyquist_param(nx, ny, tx, ty)
  call spectr(ini, tx, x, ty, y, ofac, n50, iwin, freq, gxx, gyy, gxy, cxy, phxy)
  !
...
subroutine rx_setdim(nx, ny, tx, ty, cfg_ofac, cfg_hifac, cfg_n50, nout)
  use param
  use nyquist
  use redfit_x_module
  implicit none
  integer, intent(in) :: nx, ny, cfg_n50
  real(dp), intent(in) :: tx(nx), ty(ny)
  real(dp), intent(in) :: cfg_ofac, cfg_hifac
  integer, intent(out) :: nout
  n50 = cfg_n50
  ofac = cfg_ofac
  hifac = cfg_hifac
  print *, 'call calc_nyquist_param from rx_setdim'
  print *, 'ny, ty(1), ty(ny)=', ny, ty(1), ty(ny)
  call calc_nyquist_param(nx, ny, tx, ty)
  nout = nfreq
end subroutine rx_setdim
