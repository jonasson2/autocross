...
contains
  subroutine count_series_files(fnin, npx, npy)
    use precision
  ...
  end subroutine count_series_files

  subroutine calc_nyquist_param(npx, npy, tx, ty)
    use param
    use nyquist
    ...
    real(dp), intent(in) :: tx(npx), ty(npy)
    ...
    ! ----------------------------------------------------------
    print *, 'npy, ty(1), ty(npy)=', npy, ty(1), ty(npy)
    avgdtx = (tx(npx) - tx(1)) / (npx - 1)    ! avg. sampling interval for x
    avgdty = (ty(npy) - ty(1)) / (npy - 1)    ! avg. sampling interval for y
    !
    ...
    lfreq = nfreq * 2
    print *, 'nfreq=', nfreq
  end subroutine calc_nyquist_param
  
  !--------------------------------------------------------------------------
  subroutine readdat(fnin, npx, npy, tx, x, ty, y)
    !--------------------------------------------------------------------------
    use precision
    use const,only:n_fnin
    !
